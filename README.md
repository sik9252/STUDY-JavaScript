...
## 변수 이용해서 Dark/Light 모드 만들기

```html
<button id="btn" class="btn btn-dark rounded-pill text-white">
  <span id="mode">Dark</span> 모드로 바꾸기
</button>
```

먼저 위와 같이 누르면 Dark 모드와 Light 모드로 변경할 수 있는 버튼을 생성해준다.

구현할 기능의 조건은 다음과 같다.

1. `Dark 모드로 바꾸기`라고 써있는 버튼을 누르면 배경 색상을 어둡게 바꿔주고, 버튼의 text를 Light 모드로 바꾸기로 변경한다.

2. `Light 모드로 바꾸기`라고 써있는 버튼을 누르면 배경 색상을 밝게 바꿔주고, 버튼의 text를 다시 Dark 모드로 바꾸기로 변경한다.

3. count 라는 변수를 이용해 버튼을 한번 누를때마다 count를 증가시켜 count가 홀수일때 Dark 모드가 활성화되고, 짝수일때 Light 모드가 활성화되도록 구현할 예정이다.
...
위와 같이 작성하면 1초 `후에` console에 안녕이 출력된다.

**주의점**

`콜백 함수를 넘길때`, `함수명 뒤에 ()를 붙이면 안된다.` setTimeout과 같은 경우 함수의 참조 값을 받도록 정의되어 있는데 함수()를 전달해버리면 `함수 실행 결과가 전달`되어 버린다. 이런 경우 return이 없는 함수라면 undefined가 호출되어 `함수가 동작하지 않게 된다`.

타이머를 취소하고 싶을때는 `clearTimeout()`을 사용하면 되는데 인자로 setTimeout()을 실행할 변수명을 전달하면 된다.
...
위와 같이 작성하면 1초 `마다` console에 안녕이 출력된다.

마찬가지로 중단하려면 `clearInterval()`을 사용하면 된다.

**setTimeout()의 중첩 사용**

setTimeout()을 중첩 사용하면 setInterval()을 사용한 것처럼 일정 간격을 두고 무언가를 실행시킬수 있는데 호출 결과에 따라 `다음 호출을 원하는 방식으로 조정해 스케줄링` 할 수 있기 때문에 setInterval()을 단독 사용하는 것보다 유연하다.

ex) 서버에 5초 간격으로 요청을 보내고 있었는데 만약 서버가 과부하 상태가 된다면 요청 간격을 10초, 20초로 늘리는 행동을 구현할수 있음.

또한 `setInterval()은 만일 해당 콜백함수를 실행하는데 걸리는 시간이 명시한 지연시간보다 길어지는 경우` 콜백함수가 종료되고 스케줄러를 확인하였을때 지연시간이 지나있으므로 `다음 호출을 바로 시작하기 때문에 모든 함수가 쉴새 없이 계속 호출되는 현상이 발생`할수 있다.

따라서 중첩 setTimeout()을 사용하면 지연 간격이 보장되므로 더욱 안전하다.
